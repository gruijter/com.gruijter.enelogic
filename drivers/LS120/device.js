...
class LS120Device extends Device {
      ...
      for (let index = 0; index <= correctCaps.length; index += 1) {
        ...
        if (caps[index] !== newCap) {
          ...
          if (newCap !== undefined) {
            ...
            if (state[newCap] !== undefined) await this.setCapability(newCap, state[newCap]).catch((error) => this.error(error));
            await setTimeoutPromise(2 * 1000); // wait a bit for Homey to settle
          }
        }
      }
  ...
  async updateDeviceState(meters) {
    // console.log(`updating states for: ${this.getName()}`, meters);
    try {
